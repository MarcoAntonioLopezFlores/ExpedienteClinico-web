...
import AcupunturaDetails from '../../components/doctor/AcupunturaDetails';
export default class ListAcupuntura extends Component {
    constructor(props) {
        super(props)
        this.state = {
            cuestionario: {},
    ...
    recuperarCuestionarios = async () => {
        ...
        const listCuestionario = await fetch(direccionIP + 'acupuntura/getAll/' + localStorage.getItem("idForExpediente"), {
            method: 'GET',
            headers: {
    ...
    realizar = () => {
        this.props.history.push("acupuntura/cuestionario")
    }
    dateFormat = (fechaDoc) => {
    ...
    searchCuestionario = (index) => {
        console.log(this.state.cuestionarios[index])
        this.setState({

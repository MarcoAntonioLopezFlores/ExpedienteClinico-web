...
export default class InsertAntecedente extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    return (
        <div>
            <Dialog fullScreen open={this.props.stateModal} onClose={this.props.toogle} >
                <AppBar style={{ backgroundColor: "#6B4C35" }} >
                    <Toolbar>
                        <IconButton edge="start" color="inherit" onClick={this.props.toogle} aria-label="close">
                            <CloseIcon />
                        </IconButton>
...
ANTICOAGULANTES? </Label>
                ...
                <Row style={{ marginTop: 20 }}>
                        ...
                        <Button onClick={this.handleClick} type="button" className="btn btn-md btn-block" style={{ backgroundColor: "#C9D468", fontWeight: "bold", borderBottomWidth: 3, borderColor: "green" }}>{this.state.textButton}</Button>
                            </div>
                        </Row>
    ...
    handleClick = async () => {
        ...
        if(responseCreateExpediente.status===200){
            ...
            swal({
                title: "¡ERROR!",
                text: "El paciente ya ha registrado sus antecedentes clínicos",
                icon: "error",
              });
              this.props.toogle() 
        }

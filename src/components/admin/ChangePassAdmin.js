...
export default class ChangePassAdmin extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            actualContrasena: "",
        ...
        if (this.state.actualContrasena !== "" && this.state.nuevaContrasena !== "" && this.state.confirmarContrasena !== "") {
            if(this.state.confirmarContrasena===this.state.nuevaContrasena){
               ...
               console.log(informacion)
                const response = await fetch(direccionIP+"updatePassAdministrador/"+JSON.parse(localStorage.getItem('usuario')).id, {
                method: 'POST',
                headers: {

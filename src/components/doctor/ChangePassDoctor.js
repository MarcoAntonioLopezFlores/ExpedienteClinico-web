...
export default class ChangePassDoctor extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            actualContrasena: "",
    ...
    actualizarPass = async() => {
        ...
        if (this.state.actualContrasena !== "" && this.state.nuevaContrasena !== "" && this.state.confirmarContrasena !== "") {
            if(this.state.confirmarContrasena===this.state.nuevaContrasena){
                ...
                const response = await fetch(direccionIP+"updatePassDoctor/"+JSON.parse(localStorage.getItem('usuario')).id, {
                method: 'POST',
                headers: {
